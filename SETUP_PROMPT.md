...
11. Run:
   ...
   Then verify the generated Codex skills:
   ...
   - `sync-skills` should use Codex headless mode (`codex exec`) with a fast model to generate useful skill descriptions for that frontmatter. If I need a different fast model, set `CLAUDE_TO_CODEX_FRONTMATTER_MODEL`; otherwise use the project default.
   - Generated skill wrappers should include a source hash so rerunning setup does not rewrite unchanged skills.
...
14. Show me how to uninstall safely with `cwc --uninstall --dry-run` and `cwc --uninstall --yes`. Explain that uninstall removes only claude-to-codex-owned commands, generated skill wrappers, and the `claude-bridge` MCP entry when it points at `claude-to-codex`.
